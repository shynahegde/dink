...
function AddModal({ title, fields, onSave, onClose, sz }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:100, background:"rgba(0,0,0,.4)", display:"flex", alignItems:"flex-end", justifyContent:"center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:"24px 24px 0 0", width:"100%", maxWidth:480, padding:"24px 20px", paddingBottom:"max(24px, env(safe-area-inset-bottom))", maxHeight:"85vh", overflowY:"auto", animation:"fadeUp .25s ease-out" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
          <div style={{ fontSize:sz.xl, fontWeight:800 }}>{title}</div>
          <div onClick={onClose} style={{ width:36, height:36, borderRadius:10, background:"#f1f5f9", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:18, color:"var(--text3)" }}>✕</div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {fields.map(f => (
            <div key={f.key}>
              <div style={{ fontSize:sz.xs, fontWeight:700, color:"var(--text3)", textTransform:"uppercase", letterSpacing:1, marginBottom:4 }}>{f.label}</div>
              {f.type === "select" ? (
                <select value={form[f.key]||""} onChange={e => setForm({...form, [f.key]:e.target.value})} style={{ width:"100%", padding:"12px 14px", borderRadius:12, border:"2px solid var(--border)", fontSize:sz.base, fontFamily:"'Nunito',sans-serif", background:"#f9fafb", outline:"none" }}>
                  <option value="">Select...</option>
                  {f.options.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              ) : (
                <input type={f.type||"text"} placeholder={f.placeholder||""} value={form[f.key]||""} onChange={e => setForm({...form, [f.key]:e.target.value})} style={{ width:"100%", padding:"12px 14px", borderRadius:12, border:"2px solid var(--border)", fontSize:sz.base, fontFamily:"'Nunito',sans-serif", background:"#f9fafb", outline:"none" }} onFocus={e => e.target.style.borderColor="#22c55e"} onBlur={e => e.target.style.borderColor="#e5e7eb"} />
              )}
            </div>
          ))}
        </div>
        <Btn primary full sz={sz} onClick={() => onSave(form)} style={{ marginTop:20 }}>Save</Btn>
      </div>
    </div>
  );
}
...
// ── Empty State ──────────────────────────────────────────
function EmptyState({ icon, title, subtitle, btnLabel, onAdd, sz }) {
  return (
    <div className="anim" style={{ textAlign:"center", padding:"40px 20px" }}>
      <div style={{ color:"var(--text3)", marginBottom:12, opacity:.5 }}>{icon(48)}</div>
      <div style={{ fontSize:sz.lg, fontWeight:800, color:"var(--text)", marginBottom:4 }}>{title}</div>
      <div style={{ fontSize:sz.sm, color:"var(--text3)", marginBottom:20, lineHeight:1.5 }}>{subtitle}</div>
      <Btn primary sz={sz} onClick={onAdd} style={{ margin:"0 auto" }}>{I.Plus(16)} {btnLabel}</Btn>
    </div>
  );
}
...
export default function DinkApp() {
  ...
  const [showAdd, setShowAdd] = useState(null); // "tournament" | "partner" | "match" | "gear" | null
  const [loading, setLoading] = useState(true);

  // Fetch data from Firestore
  const fetchData = async (collection, setter) => {
    try {
      const res = await fetch(`/api/data?collection=${collection}`);
      const data = await res.json();
      if (data.items) setter(data.items);
  ...
  useEffect(() => {
    if (screen === "app" && user) {
      Promise.all([
        ...
        fetchData("gear", (items) => { if (items.length > 0) setGear(items); }),
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            try {
              const res = await API.courts(pos.coords.latitude, pos.coords.longitude);
              if (res.courts) setCourts(res.courts);
            } catch (e) { console.error("Courts fetch error:", e); }
          },
          () => { /* Geolocation denied — courts tab will show empty state */ }
        );
      }
    }
  }, [screen, user]);

  const saveItem = async (collection, data, setter) => {
    try {
      ...
      if (saved.id) {
        setter(prev => [{ id: saved.id, ...data, createdAt: new Date().toISOString() }, ...prev]);
      }
    } catch (e) { console.error("Save error:", e); }
    setShowAdd(null);
  };

  const deleteItem = async (collection, id, setter) => {
    try {
      await fetch(`/api/data?collection=${collection}&id=${id}`, { method: "DELETE" });
  ...
  const wr = matches.length > 0 ? Math.round(matches.filter(m=>m.r==="W").length/matches.length*100) : 0;
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", color:"var(--text)", fontFamily:"'Nunito',sans-serif", maxWidth:480, margin:"0 auto", position:"relative" }}>
      ...
      <main style={{ padding:"10px 16px 120px" }}>
        ...
        {tab === "tournaments" && <>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
            {[["Upcoming",tournaments.filter(t=>t.status==="registered").length,"var(--green)"],["Win Rate",wr+"%","#d97706"],[region==="US"?"DUPR":"UKPA",region==="US"?"--":"--","#2563eb"]].map(([l,v,c],i) => (
          ...
          {/* Next up */}
          {(() => { const n=tournaments.filter(t=>t.status==="registered").sort((a,b)=>new Date(a.date)-new Date(b.date))[0]; if(!n) return null; const d=daysTo(n.date); return (
              ...
              <div style={{ position:"relative",zIndex:1 }}>
                ...
                <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                  ...
                  <div style={{ fontSize:sz.sm, color:"#dcfce7", lineHeight:1.5 }}>
                    <div>Partner: <b>{n.partner||"TBD"}</b></div>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            ...
            <Btn sz={sz} onClick={() => setShowAdd("tournament")} style={{ padding:"8px 14px",fontSize:sz.xs }}>{I.Plus(14)} Add</Btn>
          </div>

          {tournaments.length === 0 && !loading ? (
            <EmptyState icon={I.Trophy} title="No tournaments yet" subtitle="Add your first tournament to start tracking events, deadlines, and partners." btnLabel="Add Tournament" onAdd={() => setShowAdd("tournament")} sz={sz} />
          ) : tournaments.map(t => { const sc=stColor[t.status]||stColor.open; const d=daysTo(t.date); const sel=selTourn===t.id; return (
            <Card key={t.id} onClick={() => setSelTourn(sel?null:t.id)} style={{ marginBottom:8, borderLeft:`4px solid ${tierC[t.tier]||"var(--green)"}`, padding:18 }}>
              ...
              {sel && (
                <div style={{ marginTop:14, paddingTop:14, borderTop:"1px solid #f1f5f9", animation:"fadeUp .2s ease-out" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14, fontSize:sz.sm }}>
                    <div><span style={{ color:"var(--text3)" }}>Partner: </span><b>{t.partner||"TBD"}</b></div>
                    <div><span style={{ color:"var(--text3)" }}>Tier: </span><b style={{ color:tierC[t.tier]||"var(--green)" }}>{t.tier}</b></div>
                    <div><span style={{ color:"var(--text3)" }}>Deadline: </span><b>{t.dl?fmtDate(t.dl):"--"}</b></div>
                    <div><span style={{ color:"var(--text3)" }}>Fee: </span><b>{t.fee||"--"}</b></div>
                  </div>
                  <div style={{ display:"flex", gap:8 }}>
                    {t.status==="open" && <Btn primary full sz={sz}>Register Now</Btn>}
                    <Btn full sz={sz} onClick={(e) => { e.stopPropagation(); deleteItem("tournaments", t.id, setTournaments); }} style={{ background:"#fef2f2", border:"1px solid #fecaca", color:"#dc2626" }}>Delete</Btn>
                  </div>
                </div>
              )}
            </Card>
        ...
        {tab === "partners" && <>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            ...
            <Btn sz={sz} onClick={() => setShowAdd("partner")} style={{ padding:"8px 14px",fontSize:sz.xs }}>{I.Plus(14)} Add</Btn>
          </div>
          {partners.length === 0 && !loading ? (
            <EmptyState icon={I.Users} title="No partners yet" subtitle="Add your doubles and mixed partners to track records, availability, and chemistry." btnLabel="Add Partner" onAdd={() => setShowAdd("partner")} sz={sz} />
          ) : partners.map((p,i) => { const ac=accents[i%5]; const av=p.name?p.name.split(" ").map(w=>w[0]).join("").slice(0,2):"??"; return (
            <Card key={p.id} style={{ marginBottom:8, padding:18 }}>
              <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                <div style={{ width:sz.tap+4, height:sz.tap+4, borderRadius:14, background:`${ac}15`, border:`2px solid ${ac}30`, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'JetBrains Mono'", fontWeight:700, fontSize:sz.sm, color:ac }}>{av}</div>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <span style={{ fontSize:sz.base, fontWeight:700 }}>{p.name}</span>
                    <Tag style={{ fontSize:10 }}>{p.type||"Doubles"}</Tag>
                  </div>
                  <div style={{ fontSize:sz.sm, color:"var(--text3)", marginTop:2 }}>{p.avail||"Flexible"} · {p.phone||""}</div>
                </div>
              ...
              <div style={{ display:"flex", gap:8, marginTop:12 }}>
                <Btn full sz={sz} style={{ background:"#eff6ff", border:"1px solid #bfdbfe", color:"#1d4ed8" }}>{I.Phone(16)} Message</Btn>
                <Btn sz={sz} onClick={() => deleteItem("partners", p.id, setPartners)} style={{ background:"#fef2f2", border:"1px solid #fecaca", color:"#dc2626", padding:"8px 14px" }}>✕</Btn>
              </div>
            </Card>
          );})}
        </>}
        ...
        {tab === "matches" && <>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            <div style={{ fontSize:sz.lg, fontWeight:800 }}>Match History</div>
            <Btn sz={sz} onClick={() => setShowAdd("match")} style={{ padding:"8px 14px",fontSize:sz.xs }}>{I.Plus(14)} Add</Btn>
          </div>
          {matches.length === 0 && !loading ? (
            <EmptyState icon={I.Chart} title="No matches recorded" subtitle="Log your match results to track win rates, partner performance, and analytics." btnLabel="Log Match" onAdd={() => setShowAdd("match")} sz={sz} />
          ) : matches.map(m => (
          ...
          {/* Analytics */}
          {matches.length > 0 && <Card style={{ marginTop:8 }}>
            <div style={{ fontSize:sz.base, fontWeight:700, marginBottom:12 }}>Quick Analytics</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {[
                ["Total Matches", matches.length, "var(--text)"],
                ["Win Rate", wr+"%", wr>=60?"var(--green)":"#d97706"],
                ["Wins", matches.filter(m=>m.r==="W").length, "var(--green)"],
                ["Losses", matches.filter(m=>m.r==="L").length, "#dc2626"],
              ].map(([l,v,c],i) => (
        ...
        {tab === "courts" && <>
          ...
          {courts.length === 0 ? (
            <EmptyState icon={I.MapPin} title="Finding nearby courts..." subtitle="Allow location access to discover pickleball courts near you, or check that GOOGLE_MAPS_API_KEY is set." btnLabel="Retry" onAdd={() => {
            ...
            <Card key={c.id} style={{ marginBottom:8, padding:18 }}>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:sz.sm, color:"var(--text3)", marginBottom:8 }}>{c.address}</div>
                </div>
                {c.rating && <div style={{ textAlign:"right", marginLeft:12, flexShrink:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:3, justifyContent:"flex-end", color:"#eab308" }}>{I.Star()}<span style={{ fontFamily:"'JetBrains Mono'", fontSize:sz.base, fontWeight:700, color:"#92400e" }}>{c.rating}</span></div>
                  <div style={{ fontSize:sz.xs, color:"var(--text3)", marginTop:2 }}>{c.totalRatings} reviews</div>
                </div>}
              </div>
              <Btn full sz={sz} onClick={() => window.open(`https://www.google.com/maps/place/?q=place_id:${c.id}`, "_blank")} style={{ marginTop:12, background:"#eff6ff", border:"1px solid #bfdbfe", color:"#1d4ed8" }}>
                {I.MapPin(16)} Navigate
              </Btn>
            </Card>
          ))}
        </>}
        ...
        {tab === "gear" && <>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <Btn sz={sz} onClick={() => setShowAdd("gear")} style={{ padding:"8px 14px",fontSize:sz.xs }}>{I.Plus(14)} Add</Btn>
            </div>
          </div>
      ...
      {/* ── ADD MODALS ── */}
      {showAdd === "tournament" && (
        <AddModal title="Add Tournament" sz={sz} onClose={() => setShowAdd(null)}
          fields={[
            { key:"name", label:"Tournament Name", placeholder:"e.g. APP Chicago Open" },
            { key:"date", label:"Date", type:"date" },
            { key:"loc", label:"Location", placeholder:"City, State" },
            { key:"partner", label:"Partner Name", placeholder:"Partner name or TBD" },
            { key:"fmt", label:"Format", type:"select", options:["Bracket","Pool Play","Round Robin"] },
            { key:"surface", label:"Surface", type:"select", options:["Indoor","Outdoor"] },
            { key:"tier", label:"Tier", type:"select", options:["APP Tour","PPA","UKPA","Local","Regional"] },
            { key:"status", label:"Status", type:"select", options:["registered","waitlisted","open"] },
            { key:"fee", label:"Entry Fee", placeholder:"$85" },
            { key:"dl", label:"Registration Deadline", type:"date" },
          ]}
          onSave={data => saveItem("tournaments", data, setTournaments)}
        />
      )}
      {showAdd === "partner" && (
        <AddModal title="Add Partner" sz={sz} onClose={() => setShowAdd(null)}
          fields={[
            { key:"name", label:"Name", placeholder:"Full name" },
            { key:"dupr", label:region==="US"?"DUPR Rating":"UKPA Rating", placeholder:"e.g. 4.5" },
            { key:"type", label:"Type", type:"select", options:["Doubles","Mixed"] },
            { key:"avail", label:"Availability", type:"select", options:["Weekends","Weekdays","Flexible"] },
            { key:"phone", label:"Phone/Email", placeholder:"Contact info" },
          ]}
          onSave={data => saveItem("partners", { ...data, dupr: parseFloat(data.dupr)||0 }, setPartners)}
        />
      )}
      {showAdd === "match" && (
        <AddModal title="Log Match" sz={sz} onClose={() => setShowAdd(null)}
          fields={[
            { key:"tourn", label:"Tournament", placeholder:"Tournament name" },
            { key:"date", label:"Date", type:"date" },
            { key:"partner", label:"Your Partner", placeholder:"Partner name" },
            { key:"opp", label:"Opponents", placeholder:"e.g. Smith/Jones" },
            { key:"score", label:"Score", placeholder:"e.g. 11-7, 11-9" },
            { key:"r", label:"Result", type:"select", options:["W","L"] },
            { key:"surface", label:"Surface", type:"select", options:["Indoor","Outdoor"] },
          ]}
          onSave={data => saveItem("matches", data, setMatches)}
        />
      )}
      {showAdd === "gear" && (
        <AddModal title="Add Gear Item" sz={sz} onClose={() => setShowAdd(null)}
          fields={[
            { key:"item", label:"Item Name", placeholder:"e.g. Wristbands" },
            { key:"cat", label:"Category", type:"select", options:["Equipment","Apparel","Nutrition","Other"] },
          ]}
          onSave={data => { const newItem = { id:"g"+Date.now(), ...data, c:false }; setGear(prev => [...prev, newItem]); setShowAdd(null); }}
        />
      )}
